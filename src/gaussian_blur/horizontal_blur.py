...
from src.gaussian_blur.horizontal_blur_shader import HorizontalBlurShader


class HorizontalBlur:

    def __init__(self, target_fbo_width: int, target_fbo_height: int):
        self.__shader = HorizontalBlurShader()
        self.__shader.start()
        self.__shader.load_target_width(target_fbo_width)
        self.__shader.stop()
