...
class NormalMappingRenderer:
    """
    This Class is responsible for rendering every entity object on the screen
    """
    def __init__(self, projection_matrix, shader):
    ...
    def render(self, entities: dict, clip_plane: list[float], lights: list, camera):
        self.prepare(clip_plane, lights, camera)
    ...
    def prepare_textured_model(self, model):
        ...
        glEnableVertexAttribArray(3)  # we have put the tangents in the 3rd address
        ...
        glBindTexture(GL_TEXTURE_2D, model.get_texture().get_normal_map())
    ...
    def unbind_textured_model(self):
        ...
        glDisableVertexAttribArray(3)  # disable the attributeList after using it
    ...
    def prepare(self, clip_plane: list[float], lights: list, camera):
        self.__shader.load_clip_plane(clip_plane)
        self.__shader.load_sky_color(*DisplayManager.get_backdrop_color())
        view_matrix = Maths.create_view_matrix(camera)
        self.__shader.load_lights(lights, view_matrix)
